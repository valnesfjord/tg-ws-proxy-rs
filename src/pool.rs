...
use crate::ws_client::{connect_cf_ws_for_dc, connect_ws_for_dc, TgWsStream};
...
// ─── Cloudflare WebSocket connection pool ─────────────────────────────────────

/// Pre-warmed WebSocket connection pool for the Cloudflare proxy path.
///
/// Cloudflare suffers from a "cold start" on the first WebSocket connection to
/// each origin: CF must establish its own TCP connection to the Telegram server
/// before forwarding data.  This typically takes 100–400 ms — long enough for
/// the Telegram Desktop client to time out and close the connection before any
/// data flows (resulting in sessions that close with 0 bytes received).
///
/// Maintaining a small pool of idle CF WebSocket connections eliminates this
/// cold-start penalty: subsequent clients reuse an already-warmed connection
/// where CF's backend to Telegram is already open.
pub struct CfPool {
    pool_size: usize,
    max_age: Duration,
...
impl CfPool {
    ...
    /// Take a pre-warmed CF connection from the pool, if available and fresh.
    ///
    /// Returns `Some(ws)` on a pool hit, `None` if the bucket is empty or all
    /// entries were stale.  Schedules a background refill either way.
    pub async fn get(self: &Arc<Self>, dc: u32, is_media: bool) -> Option<TgWsStream> {
        let now = Instant::now();
        let mut lock = self.idle.lock().await;
        let bucket = lock.entry((dc, is_media)).or_default();
        ...
        while let Some(mut entry) = bucket.pop() {
            ...
            // Non-blocking liveness check: discard connections that the server
            // has already closed (Cloudflare idle timeout or a Ping we missed).
            if entry.ws.next().now_or_never().is_some() {
                debug!(
                    "cf-pool: discarding stale DC{}{} connection",
                    dc,
                    if is_media { "m" } else { "" }
                );
                continue;
    ...
    /// Warm up the pool for DCs that will use CF as their primary path.
    ///
    /// - When `--cf-priority` is set, DCs listed in `--dc-ip` use CF first.
    /// - DCs *not* listed in `--dc-ip` always use CF first (Path 1 in the
    ///   routing logic), so they are warmed up unconditionally.
    pub async fn warmup(&self, config: &Config) {
        let dc_redirects = config.dc_redirects();

        // Collect DCs that need CF warm-up.
        let mut dcs_to_warmup: Vec<u32> = Vec::new();

        if config.cf_priority {
            // DCs with --dc-ip use CF first when cf_priority is set.
            dcs_to_warmup.extend(dc_redirects.keys().copied());
        }

        // DCs without --dc-ip always use CF as primary when CF domains are
        // configured.  Include all standard Telegram DCs that the user might
        // not have given explicit --dc-ip entries for.
        // DC 203 is a media-only relay DC (distinct from the main DCs 1–5)
        // commonly used by Telegram clients for file transfers.
        for dc in [1u32, 2, 3, 4, 5, 203] {
            if !dc_redirects.contains_key(&dc) && !dcs_to_warmup.contains(&dc) {
                dcs_to_warmup.push(dc);
            }
        }

        for dc in dcs_to_warmup {
            for is_media in [false, true] {
                let new_conns = Self::connect_batch_cf(
                    &self.cf_domains,
                    dc,
                    is_media,
        ...
        debug!("CF pool warmup complete");
    }

    // ── Internal ─────────────────────────────────────────────────────────

    async fn refill(self: &Arc<Self>, dc: u32, is_media: bool) {
        let registered = self.refilling.lock().unwrap().insert((dc, is_media));
        if !registered {
            return;
        }
    ...
    async fn connect_batch_cf(
        ...
        for _ in 0..count {
            match connect_cf_ws_for_dc(dc, cf_domains, is_media, skip_tls, timeout).await {
                (Some(ws), _) => results.push(ws),
                (None, _) => {
                    warn!(
                        "cf-pool: failed to pre-connect DC{}{}",
                        dc,
                        if is_media { "m" } else { "" }
                    );
                    break;
